...
func NewBinlogHandler(ctx context.Context, cfg config.Config, esClient *elasticsearch.Client, positionManager *position.PositionManager, syncTaskConfig config.SyncTask, source config.DataSource, target config.DataSource) *BinlogHandler {
...
func (h *BinlogHandler) Start() {
	...
	logx.Info("Starting Mysql2ES binlog event handler")
	...
	// 等待停止信号
	<-h.ctx.Done()
	logx.Info("Mysql2ES binlog event handler stopped")
}
...
func (h *BinlogHandler) Stop() {
	...
	logx.Info("Stopping Mysql2ES binlog event handler")
	h.cancel()
	...
	select {
	case <-done:
		logx.Info("Mysql2ES binlog event handler stopped successfully")
...
func (h *BinlogHandler) processInsertBatches() {
	...
	tableData := make(map[string][]elasticsearch.IndexDocument) // 修改为 IndexDocument 类型
	...
	for {
		...
		case data, ok := <-h.insertQueue:
			...
			tableKey := mapper.config.TargetTable // Elasticsearch 使用表名作为索引名

			// 创建 IndexDocument
			docID := h.extractDocumentID(data.PrimaryKey, mapper)
			indexDoc := elasticsearch.IndexDocument{
				ID:    docID,
				Index: tableKey,
				Body:  mappedData,
			}

			tableData[tableKey] = append(tableData[tableKey], indexDoc)

			if int64(len(batch)) >= h.batchSize {
				h.writeBatch("insert", batch, tableData)
				batch = make([]*RowData, 0, h.batchSize)
				tableData = make(map[string][]elasticsearch.IndexDocument) // 修改类型
			}
...
func (h *BinlogHandler) extractDocumentID(primaryKey map[string]interface{}, mapper *FieldMapper) string {
	...
	if len(primaryKey) > 1 {
		...
		for _, k := range keys {
			parts = append(parts, fmt.Sprintf("%v", primaryKey[k]))
		}
...
// writeBatch 写入批量数据到Elasticsearch
func (h *BinlogHandler) writeBatch(action string, batch []*RowData, tableData interface{}) {
	...
	threading.GoSafe(func() {
		...
		case "insert", "update":
			// 处理插入和更新
			data := tableData.(map[string][]elasticsearch.IndexDocument) // 修改类型
			for index, documents := range data {
				if len(documents) == 0 {
					continue
				}

				// 执行批量索引 - 注意参数顺序：先docs，后index
				resp, err := h.esClient.BulkIndex(index, documents)
				if err != nil {
					...
					if resp != nil && resp.Errors {
						...
						successCount += len(documents) - failedCount
						errorCount += failedCount
						if failedCount > 0 {
							logx.Errorf("Partial failure in %s batch to %s: %d failed out of %d",
								action, index, failedCount, len(documents))
						}
					} else {
		...
		case "delete":
			// 处理删除 - 这里需要根据实际的BulkDelete函数定义来调整
			...
			for index, documentIDs := range data {
				...
				// 创建删除文档
				var deleteDocs []elasticsearch.IndexDocument
				for _, docID := range documentIDs {
					deleteDocs = append(deleteDocs, elasticsearch.IndexDocument{
						ID:    docID,
						Index: index,
					})
				}
				...
				// 执行批量删除
				resp, err := h.esClient.BulkIndex(index, deleteDocs) // 假设使用相同的API，但需要确认
				if err != nil {
					...
					if resp != nil && resp.Errors {
						...
						successCount += len(documentIDs) - failedCount
						errorCount += failedCount
					} else {
