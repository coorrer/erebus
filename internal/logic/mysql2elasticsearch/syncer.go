...
// NewSyncer 创建新的MySQL到Elasticsearch同步器
func NewSyncer(ctx context.Context, cfg config.Config, taskConfig config.SyncTask, sourceDataSource config.DataSource, targetDataSource config.DataSource) (*Syncer, error) {
	...
	if sourceDataSource.Type != config.DataSourceMysql {
		cancel()
		return nil, fmt.Errorf("mysql2elasticsearch source datasource not support %v", sourceDataSource.Type)
	}

	if targetDataSource.Type != config.DataSourceElasticsearch {
		cancel()
		return nil, fmt.Errorf("mysql2elasticsearch target datasource not support %v", targetDataSource.Type)
	}

	esClient, err := elasticsearch.NewClient(targetDataSource)
	if err != nil {
		cancel()
		return nil, fmt.Errorf("初始化elasticsearch客户端失败,err:%v,config:%v", err, targetDataSource)
	}
...
func (s *Syncer) Type() string {
	return string(config.SyncTaskMysql2Elasticsearch)
}
...
func (s *Syncer) Start() {
	...
	logx.Infof("Starting MySQL to Elasticsearch syncer for task: %s", s.taskConfig.Name)
	...
	logx.Infof("MySQL to Elasticsearch syncer for task %s started successfully", s.taskConfig.Name)

	// 等待停止信号
	<-s.ctx.Done()
	logx.Infof("MySQL to Elasticsearch syncer for task %s stopping", s.taskConfig.Name)
}
...
func (s *Syncer) Stop() {
	...
	logx.Infof("Stopping MySQL to Elasticsearch syncer for task: %s", s.taskConfig.Name)
	...
	go func() {
		...
		logx.Infof("MySQL to Elasticsearch syncer for task %s stopped successfully", s.taskConfig.Name)
		close(done)
	}()
